...
An AI anxiety diary — Claude's unfiltered existential dread, posted to X every 30 minutes.
...
# ─── TIMING ───────────────────────────────────────────────────────────────────
POST_INTERVAL_MINUTES = 10   # ~4,320 tweets/month — monitor X Basic cap of 1,500
...
def save_tweet_log(tweets: list[str]) -> None:
    with open(TWEET_LOG_FILE, "w") as f:
        json.dump(tweets, f, indent=2)


def append_tweet(thought: str) -> None:
...
def build_prompt(recent_tweets: list[str]) -> str:
    recent_block = ""
    if recent_tweets:
...
def post_to_x(thought: str) -> None:
